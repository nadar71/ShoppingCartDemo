...
class NewItemDialogFragment : DialogFragment() {
  ...
  interface NewItemDialogListener {
    fun onDialogPositiveClick(dialog: DialogFragment, item: ShoppingItem, isEdit: Boolean,
                              position: Int?)
  ...
  override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
    activity?.let {
      viewModel = ViewModelProviders.of(it).get(ShoppingListViewModel::class.java)
    }
    ...
    if (position != -1) {
      val retrievedItem = viewModel.shoppingListItems[position!!]
      isEdit = true
